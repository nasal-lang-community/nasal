...
/**
 * @brief Generate a binary opcode.
 * Used for generating practically all two-operand opcodes, like `and`, `<=`, `|=`, etc.
 * One notable exception is the equality operator `==`, which is generated by genEqOp().
 * @note Binary in this context refers to the number of operands. A binary opcode is an opcode that
 * takes two (hence binary) operands. This is opposed to a unary opcode, which takes one operand.
 */
...
{
    // Keep track of where the top of the loop stack is.
    int i = p->cg->loopTop;

    // Set the "break" and "continue" instruction pointers
    // to a high, unlikely-to-be-used address to act as a sentinel value.
    // FIXME: I'm not sure why we don't just set these to -1 or NULL?
    ...
    // Return the size of the generated bytecode
    return p->cg->codesz;
}
...
/**
 * @brief Generates opcode for the ternary `?` operator.
 */
...
/**
 * @brief Generates bytecode for a generic looping construct. Used by genWhile(), genForWhile() for loop generation.
 */
...
static int parListLen(struct Token* t)
{
    ...
    // Check if LEFT(t) is NULL before accessing its members
    struct Token* left_token = LEFT(t);
    if (left_token == NULL) {
        return 0;
    }

    if (left_token->type != TOK_COMMA) {
        return 0;
    }

    return countList(left_token, TOK_COMMA);
}
...
naRef naCodeGen(struct Parser* p, struct Token* block, struct Token* arglist)
{
    ...
    for(int i=0; i<code->nArgs; i++) {
        ARGSYMS(code)[i] = cg.argSyms[i];
    }

    for(int i=0; i<code->nOptArgs; i++) {
        OPTARGSYMS(code)[i] = cg.optArgSyms[i];
    }

    for(int i=0; i<code->nOptArgs; i++) {
        OPTARGVALS(code)[i] = cg.optArgVals[i];
    }

    for(int i=0; i<code->codesz; i++) {
        BYTECODE(code)[i] = cg.byteCode[i];
    }

    for(int i=0; i<code->nLines; i++) {
        LINEIPS(code)[i] = cg.lineIps[i];
    }
    return codeObj;
}
